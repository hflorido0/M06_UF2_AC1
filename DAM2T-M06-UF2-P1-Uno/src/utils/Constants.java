...
public class Constants {
	...
	public static final String GET_ULTIMA_CARTA = "select p.id, c.numero, c.color, c.id_jugador, p.estado from partida p inner join carta c on c.id = p.id_carta where p.id = (select max(id) from partida);";
	public static final String GET_MANO_JUGADOR_POR_ID = "select c.id, c.numero, c.color, c.id_jugador, 0 from carta c left join partida p on p.id_carta = c.id where id_jugador = ? and p.id is null";
	...
	public static final String CAMBIAR_ESTADO_1 = "update partida set estado = 1 where id = ?";
